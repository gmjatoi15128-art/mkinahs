...
export function PublicLayout({ children, settings, seo: seoRecords, title, description, breadcrumbs }: PublicLayoutProps) {
  ...
  return <div className="min-h-screen bg-paper text-slate-900 pb-16 lg:pb-0">
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur-xl">
      ...
      <div className="container flex h-[76px] items-center justify-between gap-4"><Link href="/" className="flex min-w-0 items-center gap-3" aria-label={`${name} home`}><img src={logoUrl} alt="MK Institute logo" className="h-12 w-12 rounded-full object-contain shadow-sm" /><span className="max-w-[220px] text-sm font-extrabold leading-tight tracking-tight text-navy sm:max-w-sm">{name}</span></Link><nav className="hidden items-center gap-5 xl:flex" aria-label="Primary navigation">{mainNavigation.map(([label, href]) => <Link key={href} href={href} className={`nav-link ${location === href ? "nav-link-active" : ""}`}>{label}</Link>)}</nav><div className="hidden lg:block"><Link href="/admissions" className="btn-primary">Admissions Information <ArrowRight className="h-4 w-4" /></Link></div><button type="button" onClick={() => setOpen(!open)} aria-expanded={open} aria-label="Toggle menu" className="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 text-navy transition hover:border-teal-600 hover:bg-teal-50 lg:hidden">{open ? <X /> : <Menu />}</button></div>
    ...
    <div className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-3 border-t border-slate-200 bg-white p-2 shadow-[0_-8px_30px_rgba(15,23,42,0.09)] lg:hidden">{phone ? <a href={`tel:${phone}`} className="mobile-action"><Phone className="h-4 w-4" />Call</a> : <span className="mobile-action text-slate-400" aria-disabled="true"><Phone className="h-4 w-4" />Call</span>}{whatsapp ? <a href={`https://wa.me/${whatsapp.replace(/\D/g, "")}`} className="mobile-action"><Stethoscope className="h-4 w-4" />WhatsApp</a> : <span className="mobile-action text-slate-400" aria-disabled="true"><Stethoscope className="h-4 w-4" />WhatsApp</span>}<Link href="/admissions" className="mobile-action bg-navy text-white">Admissions</Link></div>
  </div>;
}

export function PageHero({ eyebrow, title, description }: { eyebrow?: string; title: string; description?: string }) {
  return <section className="page-hero"><div className="container relative z-10 max-w-4xl"><p className="eyebrow text-teal-200">{eyebrow || "MK Institute"}</p><h1 className="academic-display mt-4 text-balance text-4xl text-white sm:text-5xl">{title}</h1>{description ? <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-200">{description}</p> : null}</div></section>;
}
