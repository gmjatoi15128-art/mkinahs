...
export function PublicLayout({ children, settings, seo: seoRecords, pressReleases, title, description, breadcrumbs }: PublicLayoutProps) {
  ...
  return <div className="site-shell min-h-screen bg-paper text-slate-900 pb-16 lg:pb-0">
    ...
    <header className="site-header sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur-xl">
      ...
      <div className="container flex h-[76px] items-center justify-between gap-4"><Link href="/" className="brand-lockup flex min-w-0 items-center gap-3" aria-label={`${name} home`}><img src={logoUrl} alt="MK Institute logo" className="brand-mark h-12 w-12 rounded-full object-contain shadow-sm" /><span className="brand-wordmark max-w-[220px] text-sm font-extrabold leading-tight tracking-tight text-navy sm:max-w-sm">{name}</span></Link><nav className="hidden items-center gap-4 xl:flex" aria-label="Primary navigation">{mainNavigation.map(([label, href]) => <Link key={href} href={href} className={`nav-link ${location === href ? "nav-link-active" : ""}`}>{label}</Link>)}</nav><div className="hidden lg:block"><Link href="/admissions" className="btn-primary">Admissions Information <ArrowRight className="h-4 w-4" /></Link></div><button type="button" onClick={() => setOpen(!open)} aria-expanded={open} aria-label="Toggle menu" className="menu-trigger grid h-11 w-11 place-items-center rounded-xl border border-slate-200 text-navy transition hover:border-teal-600 hover:bg-teal-50 lg:hidden">{open ? <X /> : <Menu />}</button></div>
      {open ? <div className="border-t border-slate-200 bg-white px-4 py-4 lg:hidden"><PublicMobileMenu onNavigate={() => setOpen(false)} /></div> : null}
    </header>
